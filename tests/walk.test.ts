...
it("expect walk to throw when root is not a directory", async () => {
  ...
  }).rejects.toThrow(new Error(`Error walking path "${normalize(join(__dirname, "fixtures", "package.json"))}": ENOTDIR: not a directory, scandir '${normalize(join(__dirname, "fixtures", "package.json"))}'`));
});
...
describe("sync", () => {
  ...
  it("expect walk to throw when root is not a directory", () => {
    ...
    }).toThrow(new Error(`Error walking path "${normalize(join(__dirname, "fixtures", "package.json"))}": ENOTDIR: not a directory, scandir '${normalize(join(__dirname, "fixtures", "package.json"))}'`));
  });
